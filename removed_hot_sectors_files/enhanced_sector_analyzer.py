...
class EnhancedSectorAnalyzer:
    ...
    def analyze_hot_sectors(self, top_n: int = 10, force_refresh: bool = False) -> Dict:
        """分析热门行业
        ...
        Returns:
            包含热门行业的字典
        """
        cache_path = self._get_cache_path('hot_sectors')
        
        # 检查缓存
        if not force_refresh and self._is_cache_valid(cache_path, self.cache_expiry['hot_sectors']):
            cached_data = self._load_from_cache(cache_path)
            if cached_data:
                self.logger.info("从缓存加载热门行业")
                # 确保返回的数据包含prediction_data字段
                if 'data' in cached_data and 'prediction_data' not in cached_data['data']:
                    cached_data['data']['prediction_data'] = []
        ...
        try:
            ...
            for sector in sectors:
                try:
                    # 获取行业指数数据
                    sector_data = self._get_sector_index_data(sector['name'])
                    
                    if sector_data and 'close' in sector_data:
                        # 计算技术指标
                        sector_data = self._calculate_indicators(sector_data)
                        
                        # 计算热度分数
                        hot_score = self._calculate_hot_score(sector_data)
                        
                        # 收集资金流向数据
                        fund_flow = self._get_sector_fund_flow(sector['name'])
                        ...
                        hot_sectors.append({
                            ...
                            'technical_indicators': {
                                'rsi': sector_data['rsi'][-1] if 'rsi' in sector_data and len(sector_data['rsi']) > 0 else 50,
                                'macd_hist': sector_data['macd_hist'][-1] if 'macd_hist' in sector_data and len(sector_data['macd_hist']) > 0 else 0,
                                'volume_ratio': sector_data['volume_ratio'][-1] if 'volume_ratio' in sector_data and len(sector_data['volume_ratio']) > 0 else 1.0
                            },
                            'fund_flow': fund_flow,
                            'standard_code': sector.get('index_code', 'UNKNOWN')
                        })
                except Exception as e:
                    self.logger.error(f"处理行业 {sector['name']} 数据时出错: {str(e)}")
                    continue
            
            # 按热度分数排序
            hot_sectors = sorted(hot_sectors, key=lambda x: x['hot_score'], reverse=True)
            ...
            # 为热门行业添加简单的预测数据
            for sector in hot_sectors[:min(top_n, len(hot_sectors))]:
                default_prediction_data.append({
                    'name': sector['name'],
                    'technical_score': sector['hot_score'],  # 使用热度评分作为技术评分
                    'rotation_status': '即将轮动' if sector['hot_score'] > 75 else ''
                })
            ...
            result = {
                ...
                'data': {
                    ...
                    'prediction_data': [],
                    'prediction_data': [],
                    'prediction_data': default_prediction_data  # 确保始终包含预测数据字段
                }
            }
            ...
            return {'status': 'error', 'message': f"分析热门行业失败: {str(e)}"}
    ...
    def _get_sector_index_data(self, sector_name: str) -> Dict:
        """获取行业指数数据 (简化版)
        ...
        return {
            ...
            'vol': volumes,
            'rsi': [50 + np.random.uniform(-10, 10) for _ in range(days)],
            'macd_hist': [np.random.uniform(-1, 1) for _ in range(days)],
            'macd_dif': [np.random.uniform(-1, 1) for _ in range(days)],
            'volume_ratio': [1.0 + np.random.uniform(-0.3, 0.3) for _ in range(days)]
        }
    
    def _get_sector_fund_flow(self, sector_name: str) -> Dict:
        """获取行业资金流向数据 (简化版)
        ...
        # 生成随机流入金额
        net_inflow = np.random.normal(0, 500)
        
        return {
            'net_inflow': round(net_inflow, 2),
            'large_inflow': round(net_inflow * 0.4, 2),
            'medium_inflow': round(net_inflow * 0.3, 2),
            'small_inflow': round(net_inflow * 0.3, 2),
            'update_time': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        }
    ...
    def _calculate_indicators(self, data: Dict) -> Dict:
        ...
        Returns:
            添加了技术指标的数据
        """
        # 直接返回数据，因为我们在_get_sector_index_data中已经添加了需要的指标
        return data
    
    def _calculate_hot_score(self, data: Dict) -> float:
        """计算热度分数 (简化版)
        ...
        seed = int(hashlib.md5((sector_name + date_str).encode()).hexdigest(), 16) % 10000
        np.random.seed(seed)
        
        return round(50 + np.random.uniform(0, 30), 1) 